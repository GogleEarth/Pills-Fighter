...
void CLobbyScene::JoinPlayer(int nIndex, const char *pstrPlayerName)
{
	...
	m_ppTextObjects[m_nPlayers++] = AddText("휴먼매직체", pstrPlayerName, xmf2Pos, XMFLOAT2(1.0f, 1.0f), XMFLOAT2(1.0f, 1.0f), xmf4Color);
}
...
void CLobbyScene::LeavePlayer(int nSlotIndex, bool isPlayerIndex)
{
	...
	for (int i = 0; i < 8; i++)
	{
		if (m_ppTextObjects[i])
		{
			...
			ChangeText(m_ppTextObjects[i], "휴먼매직체", m_ppTextObjects[i]->GetText(), xmf2Pos, XMFLOAT2(1.0f, 1.0f), XMFLOAT2(1.0f, 1.0f), xmf4Color);
			m_nPlayers++;
		}
	}
}
